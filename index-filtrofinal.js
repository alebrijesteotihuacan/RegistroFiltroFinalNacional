...
async function crearPDF(registro, qrDataURL, barcodeDataURL) {
  try {
    ...
    // ║                       FIRMA                              ║
    // ╚══════════════════════════════════════════════════════════╝
    y = renderSectionTitle(y, 'Firma');

    // Texto de aceptación
    doc.setFontSize(8);
    doc.setFont('helvetica', 'italic');
    doc.setTextColor(...colors.grisMedio);
    doc.text('Acepto los Términos y Condiciones del Filtro Final Nacional 2026', margin, y);
    y += 10;

    const firmaColWidth = (contentWidth - colGap * 2) / 2;

    // Firma del Aspirante
    const firmaY = y;
    doc.setDrawColor(...colors.negro);
    doc.setLineWidth(0.5);
    doc.line(margin, firmaY + 28, margin + firmaColWidth, firmaY + 28);
    doc.setFontSize(fonts.label);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...colors.grisMedio);
    doc.text('NOMBRE Y FIRMA DEL ASPIRANTE', margin, firmaY + 34);

    // Firma del Titular
    const firmaXDer = margin + firmaColWidth + colGap * 2;
    doc.line(firmaXDer, firmaY + 28, firmaXDer + firmaColWidth, firmaY + 28);
    doc.text('NOMBRE Y FIRMA DEL TITULAR', firmaXDer, firmaY + 34);
...
function mostrarModalTerminos() {
  ...
  const checkbox = document.getElementById('checkAceptarTerminos');
  ...
  console.log('✅ Modal de términos mostrado');
}
...
function descargarPDFExito() {
  if (!Estado.registroActual) {
    mostrarNotificacion('No hay registro para descargar', 'error');
    return;
  }
  // Mostrar modal de términos y condiciones en lugar de descargar directamente
  mostrarModalTerminos();
}

// Event listeners para botones de la pantalla de éxito y modal de términos
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Checkbox de términos: habilitar/deshabilitar botón de aceptar
  const checkAceptar = document.getElementById('checkAceptarTerminos');
  const btnAceptar = document.getElementById('btnAceptarTerminos');
  if (checkAceptar && btnAceptar) {
    checkAceptar.addEventListener('change', () => {
      btnAceptar.disabled = !checkAceptar.checked;
    });
  }

  // Botón aceptar términos
  if (btnAceptar) {
    btnAceptar.addEventListener('click', aceptarTerminosYDescargar);
  }

  // Botones para cerrar modal de términos
